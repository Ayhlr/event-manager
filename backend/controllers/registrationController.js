...
const getManagerParticipants = async (req, res) => {
  try {
    ...
    for (let reg of registrations) {
      const pointsRecord = await PointsHistory.findOne({
        user: reg.user?._id,
        event: reg.event?._id
      });

      reg.pointsHistoryId = pointsRecord?._id || null;
      reg.pointsStatus = pointsRecord?.status || "none";
      reg.points = pointsRecord?.points || 0;
    }
